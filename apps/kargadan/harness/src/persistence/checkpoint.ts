...
type HydrateResult =
    | { readonly fresh: true }
    | { readonly chatJson: string; readonly diverged: boolean; readonly fresh: false; readonly sequence: number; readonly state: unknown };
type SessionFilter = {
...
class PersistenceService extends Effect.Service<PersistenceService>()('kargadan/PersistenceService', {
    effect: Effect.gen(function* () {
        ...
        const persist = Effect.fn('kargadan.persistence.persist')((input: {
            ...
            Effect.gen(function* () {
                const checkpoint = { ...input.checkpoint, stateHash: _hash(input.checkpoint.loopState) } as typeof KargadanCheckpoint.insert.Type;
        ...
        const hydrate = Effect.fn('kargadan.persistence.hydrate')((sessionId: string) =>
            SqlSchema.findOne({
                ...
                Effect.map(Option.match({
                    ...
                    onSome: (row) => ({ chatJson: row.chatJson, diverged: _hash(row.loopState) !== row.stateHash, fresh: false, sequence: row.sequence, state: row.loopState }) as HydrateResult,
                })),
            ),
        );
            ...
            SqlSchema.findAll({
                execute: () => {
                    const statusClause = filter.status !== undefined && filter.status.length > 0
                        ? sql` AND status IN ${sql.in(filter.status)}`
                        : sql``;
        ...
        return { completeSession, createSession, findResumable, hash: _hash, hydrate, listSessions, persist, sessionTrace } as const;
    }),
}) {}
