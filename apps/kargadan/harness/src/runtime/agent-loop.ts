    ...
    effect: Effect.gen(function* () {
        ...
        const run = Effect.fn('AgentLoop.handle')((input: {
            ...
            const _dispatch = (command: Envelope.Command, source: 'direct' | 'workflow' | 'compensation' = 'direct') =>
                Effect.sync(() => performance.now()).pipe(Effect.tap(() => hooks.onTool({ command, durationMs: 0, phase: 'start', result: Option.none(), source })),
                    Effect.flatMap((t0) => dispatch.execute(command).pipe(Effect.catchTag('CommandDispatchError', (e) => Effect.succeed(dispatch.buildErrorResult(command, e.errorPayload))),
                        Effect.map((r) => r as Envelope.Result), Effect.exit, Effect.tap((exit) => hooks.onTool({ command, durationMs: Math.max(0, Math.round(performance.now() - t0)),
            ...
            const _aux = (state: _LoopState, commandId: string, args: Record<string, unknown>,
                ...
                _dispatch(dispatch.buildCommand(state.identityBase, commandId, args, { attempt: state.attempt, deadlineMs: config.commandDeadlineMs, objectRefs, operationTag: opTag }));
            ...
            const plan = (state: _LoopState) => Effect.gen(function* () {
                ...
                const compaction = yield* Ref.get(chatRef).pipe(
                    ...
                    Effect.flatMap(Option.match({
                        onNone: () => Effect.succeed(Option.none<typeof _Loop.compaction.Type>()),
                ...
                const { candidates, fallbackId } = yield* ai.searchQuery({ entityTypes: ['command'], includeSnippets: false, term: input.intent },
                    { limit: Math.max(1, Math.min(_Limits.maxCandidates, input.catalog.length)) }).pipe(Effect.flatMap((raw) => S.decodeUnknown(_Loop.searchResult)(raw)),
            ...
            const verify = (state: _LoopState, _p: Effect.Effect.Success<ReturnType<typeof plan>>,
                ...
                const visual = yield* (HashSet.has(negotiatedCaps, _Aux.viewCapture) ? _aux(state, _Aux.viewCapture, config.viewCapture, 'verify.view.capture').pipe(
                    ...
                    Effect.flatMap(({ cap, summary }) => ai.model.generateObject({ prompt: [
                        ...
                        schema: _Loop.vision }).pipe(Effect.map((v) => ({ status: 'captured' as const, summary, vision: v.value })),
                ...
                const evidence: typeof _Loop.evidence.Type = { deterministicFailureClass: Option.getOrNull(failureClassification), deterministicStatus: detStatus, visualStatus: visual.status };
            ...
            const persistPhase = (state: _LoopState, planned: Effect.Effect.Success<ReturnType<typeof plan>>, exec: Effect.Effect.Success<ReturnType<typeof execute>>,
                vf: Effect.Effect.Success<ReturnType<typeof verify>>, decision: _LoopState) => {
                ...
                return _persist(chatRef, state.identityBase, snap, { durationMs: Math.max(0, Math.round(performance.now() - vf.startedAt)),
