...
const _Loop = {
    compaction: S.Struct({ estimatedTokensAfter: NonNegInt, estimatedTokensBefore: NonNegInt, mode: S.Literal('history_reset'),
        sequence: NonNegInt, targetTokens: NonNegInt, triggerTokens: NonNegInt }),
    evidence: S.Struct({ deterministicFailureClass: S.NullOr(FailureClass), deterministicStatus: ResultStatus,
        visualStatus: S.Literal('captured', 'capture_failed', 'capability_missing') }),
    scene: S.Struct({
        activeLayer:        S.Struct({ index: S.Int, name: S.String }), activeView: S.String,
        layerCount:         NonNegInt, objectCount: NonNegInt,
    ...
    state: S.Struct({
        attempt:              S.Int.pipe(S.greaterThanOrEqualTo(1)), correctionCycles: NonNegInt,
        lastCompaction:       S.optional(S.Unknown), operations:        S.Array(Operation),
        recentObservation:    S.optional(S.Unknown), sceneSummary:      S.optional(S.Unknown),
    ...
    effect: Effect.gen(function* () {
        ...
        const run = Effect.fn('AgentLoop.handle')((input: {
            ...
            Effect.gen(function* () {
                ...
                const persistWithChat = (identity: Envelope.IdentityBase, loopState: unknown, call: {
                ...
                const persistInboundEvent = (evt: Envelope.Event) =>
                    Effect.gen(function* () {
                        ...
                        const sequence = yield* Ref.modify(eventSequence, (c) => [c, c + 1] as const);
                        yield* persistWithChat(evt, { eventType: evt.eventType, sequence }, {
                        ...
                        return { eventEnvelope: evt, sequence } as const;
                    });
                const _dispatchCommand = (command: Envelope.Command) =>
                    dispatch.execute(command).pipe(
                        Effect.catchTag('CommandDispatchError', (e) => Effect.succeed(dispatch.buildErrorResult(command, e.errorPayload))),
                        Effect.map((r) => r as Envelope.Result));
                const _dispatchAux = (state: LoopState, commandId: string, args: Record<string, unknown>,
                ...
                const _compactChat = (state: LoopState, serialized: string, tokensBefore: number, target: number, trigger: number) =>
                    Effect.gen(function* () {
                        const window = Math.max(target * 4, 1_200);
                        const failureTag = state.attempt > 1 ? 'retrying' : 'steady';
                        const seed = JSON.stringify({
                            compaction: { estimatedTokensBefore: tokensBefore, targetTokens: target, triggerTokens: trigger },
                            context: { failureState: state.status === 'Failed' ? 'failed' : failureTag,
                ...
                yield* workflowEngine.register(_writeWorkflow, (payload) =>
                    Effect.gen(function* () {
                        ...
                        const approved = yield* DurableDeferred.token(gate).pipe(
                            Effect.tap((token) => DurableDeferred.succeed(gate, { token, value: true })),
                            Effect.zipRight(DurableDeferred.await(gate)));
                        yield* Effect.when(Effect.fail('write_workflow.approval.rejected'), () => !approved);
                        const result = yield* Activity.make({
                            error:   S.String,
                            execute: dispatch.execute(command).pipe(
                                Effect.mapError((e) => `write_workflow.dispatch: ${String(e)}`),
                                Effect.flatMap ((p) => _decodeEnvelope('result')(p).pipe(Effect.mapError((e) => `write_workflow.decode.result: ${e}`))),
                            ...
                            dispatch.execute(dispatch.buildCommand(
                                ...
                                { attempt: command.telemetryContext.attempt, deadlineMs: command.deadlineMs,
                                    operationTag: `workflow.compensate.${command.commandId}` })).pipe(
                                Effect.mapError((e) => `write_workflow.compensation.dispatch: ${String(e)}`),
                                Effect.flatMap ((r) => _decodeEnvelope('result')(r).pipe(
                                    Effect.mapError((e) => `write_workflow.compensation.decode.result: ${e}`))),
                                ...
                                Effect.catchAll((e) => Telemetry.emit('kargadan.workflow.compensation.failed', {
                                    commandId: command.commandId, error: String(e),
                                    workflowExecutionId: payload.workflowExecutionId })),
                                Effect.asVoid)));
                        ...
                        Effect.flatMap((ws) =>
                            _decodeEnvelope('result')(ws.result).pipe(
                                ...
                                Effect.catchAll((e) => Effect.succeed({
                                    approved: ws.approved, executionId: ws.workflowExecutionId,
                                    result: dispatch.buildErrorResult(command, { code: 'WORKFLOW_RESULT_DECODE_FAILED',
                                        details: { error: String(e) }, failureClass: 'retryable', message: 'Workflow result decode failed.' }) })))),
                        Effect.catchAll((e) => Effect.succeed({
                            approved: false, executionId: _workflowExecId(command, sequence),
                            result: dispatch.buildErrorResult(command, { code: 'WORKFLOW_EXECUTION_FAILED',
                                details: { error: String(e) }, failureClass: 'compensatable', message: 'Workflow execution failed.' }) })));
                const _probeScene = (state: LoopState, operationTag: string) =>
                ...
                const plan = (state: LoopState) =>
                    Effect.gen(function* () {
                        ...
                        yield* Effect.filterOrFail(
                            Effect.succeed(entry.params.filter((p) => p.required && !Object.hasOwn(args, p.name))),
                            (missing) => missing.length === 0,
                            (missing) => new CommandDispatchError({ details: { commandId: planned.commandId,
                                message: `Missing required parameters: ${missing.map((p) => p.name).join(', ')}` }, reason: 'protocol' }));
                        const mode = entry.isDestructive ? 'write' as const : 'read' as const;
                        const kind = entry.dispatch.mode === 'script' ? 'script' as const : mode;
                        ...
                        const writeMeta = kind === 'write'
                            ? { idempotency: persistence.idempotency(state.identityBase.correlationId, args, state.sequence), undoScope: 'kargadan.phase7' }
                            : {};
                        const command = dispatch.buildCommand(state.identityBase, planned.commandId, args, {
                            attempt: state.attempt, deadlineMs: commandDeadlineMs, ...writeMeta,
                ...
                const verify = (state: LoopState, _plan: Effect.Effect.Success<ReturnType<typeof plan>>,
                    ...
                    Effect.gen(function* () {
                        ...
                        const failureClass = Match.value({
                            cmdErr:   execution.result.status === 'error',
                            objErr:   Option.exists(objectProbe, (p) => p.status === 'error'),
                            ...
                            Match.when( (s) => s.cmdErr, () => Option.some(execution.result.error?.failureClass ?? 'fatal')),
                            Match.when( (s) => s.sceneErr || s.objErr, () => Option.some<Envelope.FailureClass>('retryable')),
                            Match.when( (s) => s.sceneBad, () => Option.some<Envelope.FailureClass>('correctable')),
                            Match.orElse(() => Option.none<Envelope.FailureClass>()));
                        ...
                        return {
                            ...
                            result: Option.some({
                                ...
                                verified:    detStatus === 'ok', visual,
                                workflow:    Option.getOrUndefined(execution.workflow) }),
                ...
                const decide = (state: LoopState, planned: Effect.Effect.Success<ReturnType<typeof plan>>,
                    ...
                    Effect.gen(function* () {
                        const fault = Option.getOrElse(verification.failureClass, () => 'fatal' as const);
                ...
                const persist = (state: LoopState, planned: Effect.Effect.Success<ReturnType<typeof plan>>,
                    execution: Effect.Effect.Success<ReturnType<typeof execute>>,
                    verification: Effect.Effect.Success<ReturnType<typeof verify>>, decision: LoopState) =>
                ...
                const baseline: LoopState = {
                    ...
                    sceneSummary: Option.some(_DefaultSceneSummary), sequence: 0, status: 'Planning',
                ...
                const initialState: LoopState = yield* Option.match(input.resume, {
                    ...
                    onSome: (r) => S.decodeUnknown(_Loop.state)(r.state).pipe(
                        Effect.map((s): LoopState => ({ ...s, command: Option.none(), identityBase: input.identityBase,
                            ...
                            sceneSummary:      Option.fromNullable(s.sceneSummary).pipe(
                                Option.filter(S.is(_Loop.scene)), Option.orElse(() => Option.some(_DefaultSceneSummary))),
                ...
                const finalState = yield* ai.runAgentCore({
                    decide, execute, initialState, isTerminal: (s) => s.status !== 'Planning', persist, plan, verify,
