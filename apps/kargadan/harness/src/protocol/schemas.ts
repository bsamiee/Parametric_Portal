...
const _FailureReason =   S.Struct({ code:    S.NonEmptyTrimmedString, failureClass: _FailureClass, message: S.NonEmptyTrimmedString });
const _ProtocolVersion = S.Struct({ major:   S.Int.pipe(S.greaterThanOrEqualTo(0)), minor: S.Int.pipe(S.greaterThanOrEqualTo(0)) });
...
const Envelope = S.Union(
    ...
    S.extend(_Identity, S.Struct({ _tag: S.Literal('handshake.ack'),    protocolVersion: _ProtocolVersion })),
    S.extend(_Identity, S.Struct({ _tag: S.Literal('handshake.reject'), reason:          _FailureReason   })),
    ...
    S.extend(_Identity, S.Struct({
        ...
        error:  S.optional(S.Struct({ details: S.optional(S.Unknown), reason: _FailureReason })),
        status: S.Literal('ok', 'error') })),
);
...
namespace Envelope {
    ...
    export type FailureReason =   typeof _FailureReason.Type;
    export type Idempotency =     NonNullable<Command['idempotency']>;
    ...
    export type ProtocolVersion = Extract<_Type, { readonly _tag: 'handshake.ack' }>['protocolVersion'];
    export type Capabilities =    Extract<_Type, { readonly _tag: 'handshake.init' }>['capabilities'];
    export type ObjectRef =       NonNullable<Command['objectRefs']>[number];
