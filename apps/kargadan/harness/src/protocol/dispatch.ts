...
class CommandDispatch extends Effect.Service<CommandDispatch>()('kargadan/CommandDispatch', {
    effect: Effect.gen(function* () {
        ...
        const { commandDeadlineMs, protocolVersion, resolveCapabilities: capabilities } = config;
                ...
                Ref.set(phaseRef, 'connecting').pipe(
                    Effect.zipRight(_request({
                        ...
                        telemetryContext: {
                            ...
                            spanId: identity.requestId.replaceAll('-', '') as typeof SpanId.Type,
                            traceId: String(identity.correlationId) as typeof TraceId.Type,
                        },
                    })),
                    Effect.flatMap((response) => Match.value(response).pipe(
                        ...
                        Match.tag('handshake.reject', (reply) => Ref.set(catalogRef, []).pipe(Effect.zipRight(
                            Effect.fail(new CommandDispatchError({
                                details: { code: reply.code, message: reply.message },
                                failureClass: reply.failureClass,
                                reason: 'rejected',
                            }))))),
        ...
        const buildCommand = (
            identityBase: Envelope.IdentityBase,
            commandId: string,
            args: Record<string, unknown>,
            options?: {
                readonly attempt?:      number;
                readonly deadlineMs?:   number;
                readonly idempotency?:  Envelope.Command['idempotency'];
                readonly objectRefs?:   Envelope.Command['objectRefs'];
                readonly operationTag?: string;
                readonly requestId?:    string;
                readonly undoScope?:    Envelope.Command['undoScope'];
            },
        ): Envelope.Command => {
            ...
            return {
                ...
                args,
                commandId,
                deadlineMs:  options?.deadlineMs ?? commandDeadlineMs,
                idempotency: options?.idempotency,
                objectRefs:  options?.objectRefs,
                requestId,
                telemetryContext: {
                    attempt:      Math.max(1, options?.attempt ?? 1),
                    operationTag: options?.operationTag ?? commandId,
                    spanId:       requestId.replaceAll('-', '') as typeof SpanId.Type,
                    traceId:      String(identityBase.correlationId) as typeof TraceId.Type,
                },
        ...
        return {
            buildCommand, buildErrorResult, close: socket.close, execute, handshake, heartbeat,
            phase: Ref.get(phaseRef), receiveCatalog, start: socket.start, takeEvent: socket.takeEvent,
        } as const;
    }),
}) {}
