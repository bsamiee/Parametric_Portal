...
const WebhooksLive = HttpApiBuilder.group(ParametricApi, 'webhooks', (handlers) =>
	Effect.gen(function* () {
		const [webhooks, database, audit] = yield* Effect.all([WebhookService, DatabaseService, AuditService]);
		...
		return handlers
				...
				.handle('test', ({ payload }) => CacheService.rateLimit('mutation', requireAdmin.pipe(
					...
					Effect.flatMap((tenantId) => webhooks.test(tenantId, new WebhookService.Endpoint({ secret: payload.secret, timeout: payload.timeout, url: payload.url })).pipe(
						Effect.catchAll((error) => Effect.fail(HttpError.Internal.of('Webhook test delivery failed', error))),
					)),
				...
				.handle('retry', ({ path }) => CacheService.rateLimit('mutation', requireAdmin.pipe(
					Effect.andThen(webhooks.retry(path.id).pipe(
						Effect.catchAll((error) => Effect.fail(HttpError.Internal.of('Webhook retry failed', error))),
					)),
					Effect.as({ success: true as const }),
