...
const _readiness = (polling: PollingService) => pipe(
    ...
    Effect.tap(({ cache, critical, db, doppler, pollingHealth, vectorConfig }) => Effect.annotateCurrentSpan({
        'health.alerts.critical':             critical.length,
        'health.alerts.total':                pollingHealth.alerts.length,
        'health.cache.connected':             cache.connected,
        'health.cache.latencyMs':             cache.latencyMs,
        'health.db.healthy':                  db.healthy,
        'health.db.latencyMs':                db.latencyMs,
        'health.doppler.consecutiveFailures': doppler.consecutiveFailures,
        'health.doppler.lastRefreshAt':       doppler.lastRefreshAt,
        'health.polling.stale':               pollingHealth.stale,
        'health.vectorConfig':                vectorConfig,
    })),
    Effect.filterOrFail(
