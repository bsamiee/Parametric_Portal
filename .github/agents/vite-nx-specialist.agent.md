...
- [USE] `nx-tools` skill for workspace queries (projects, configs, affected, generators)
  ...
  pipe(
    ...
    Effect.flatMap((c) =>
      pipe(
        Effect.all({ b, p, t, v }),
        Effect.map(({ b, p, t, v }) =>
          config[c.mode](c as never, b, { prod: p, time: t, ver: v })
        )
      )
    )
  );
...
export default defineConfig(
  Effect.runSync(createConfig({ mode: "app", name: "MyApp" }))
);
...
**Verify**: `nx run-many -t build` succeeds, `nx reset && nx build` cache hits, `nx show project` shows targets
