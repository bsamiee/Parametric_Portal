 ...
 */
import { addProjectConfiguration, formatFiles, generateFiles, joinPathFragments, names, type Tree } from '@nx/devkit';

// --- Constants (Single B) ---------------------------------------------------

const B = {
    defaults: { desc: (n: string) => `${n} library`, dir: 'packages' },
...
const normalize = (s: Schema): Opts => {
    ...
    return {
        ...
        description: s.description ?? B.defaults.desc(n.className),
        fileName: n.fileName,
...
const generator = async (tree: Tree, schema: Schema): Promise<void> => {
    ...
    addProjectConfiguration(tree, o.projectName, {
        name: o.projectName,
        projectType: B.project.type,
        root: o.projectRoot,
        sourceRoot: `${o.projectRoot}/src`,
        tags: [...o.tagList],
        targets: {},
    });
