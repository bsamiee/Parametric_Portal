...
<task type="auto">
  ...
  <action>
**Dependency note:** This task requires Plan 02-01 to be complete. Verify these exports exist:
1. `Context.Request.ClusterState` namespace type export
2. `Context.Request.makeRunnerId` static method (inlined in Request class)
...
After ctx construction, add span annotation for runner ID (before return statement):
```typescript
yield* pipe(
...
Note: `pipe` is already imported (line 9 of middleware.ts).
  </action>
  <verify>
```bash
...
<task type="auto">
  ...
  <action>
**Dependency note:** Uses `_makeShardIdString` internal helper and `RunnerId`/`ShardIdString` schemas from Plan 01 Task 1.
...
Update toAttrs to include cluster fields (add after 'client.ua', before 'fiber.id'):
```typescript
...
Note: `pipe` is already imported (line 14 of context.ts).
  </action>
  <verify>
```bash
pnpm exec nx run server:typecheck
