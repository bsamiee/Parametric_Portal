...
must_haves:
  truths:
    ...
    - "State decode errors caught and mapped to SingletonError via Effect.catchTags"
    - "Telemetry.span({ metrics: false }) + MetricsService.trackEffect for custom labels"
    - "FiberMap tracks fibers with auto-cleanup on scope close"
    ...
    - "Effect.repeatWhile for condition-based shutdown detection loops"
    - "KeyValueStore.prefix available for multi-tenant state namespacing"
  artifacts:
...
// Consolidated effect import from Plan 01 + additional patterns:
// - Cron: Schedule parsing, Cron.sequence for preview, Cron.match for validation
// - FiberMap: Fiber tracking with auto-cleanup
// - Exit: Shutdown detection via Exit.isInterrupted
// - Metric: Direct gauge/counter access (Metric.set, Metric.increment)
// - DateTime: Timestamp formatting for logs
import { Array as A, Boolean as B, Cause, Chunk, Clock, Config, Cron, Data, DateTime, Duration, Effect, Exit, FiberMap, HashMap, HashSet, Layer, Match, Metric, Number as N, Option, Ref, Schedule, Schema as S, SynchronizedRef, Tuple } from 'effect';
```

**Cron Utilities** (available from effect core — use inline where needed):
- `Cron.sequence(cron, startDateTime)` — Iterator for upcoming executions (debugging UI)
- `Cron.match(cron, dateTime)` — Verify if datetime matches schedule (manual trigger validation)
- `Cron.unsafeParse(expr)` — Parse cron expression (static strings)
- `Cron.parse(expr)` — Parse with Either result (dynamic strings needing validation)
...
**Multi-Tenant Support** (use when tenant isolation needed):
```typescript
// Wrap store with prefix for tenant-scoped state
const tenantStore = KeyValueStore.prefix(store, `tenant:${tenantId}:`);
```
  </action>
  <verify>
...
2. Ensure `Metric`, `Cron`, `Stream`, `Chunk` are in effect imports
3. Verify singleton factory provides both `_clusterLayer` and `_kvStoreLayers`
4. Verify cron factory provides `_clusterLayer`
5. Verify cron utilities use Clock.currentTimeMillis (testable)
6. Run typecheck to confirm no type errors
...
`grep -n "Cron.sequence" packages/server/src/infra/cluster.ts` returns 1+ (cron utilities).
`pnpm exec nx run server:typecheck` passes.
  </verify>
  <done>
Singleton and cron factories integrated with Context, MetricsService, KeyValueStore. Cron utilities provide schedule inspection. Typecheck passes.
  </done>
</task>
...
9. `grep -c "Cron.sequence" packages/server/src/infra/cluster.ts` returns 1+ occurrences (cron inspection)
10. `grep -c "Cron.match" packages/server/src/infra/cluster.ts` returns 1+ occurrences (schedule matching)
11. `grep -c "cronNextRuns" packages/server/src/infra/cluster.ts` returns 1+ occurrences (utility)
</verification>
...
- State errors mapped to SingletonError via Effect.catchTags
...
- Telemetry.span({ metrics: false }) + MetricsService.trackEffect for custom labels
- Heartbeat gauge updated via Clock.currentTimeMillis (testable)
- FiberMap for fiber tracking with auto-cleanup
...
- Effect.repeatWhile for condition-based shutdown detection
- Imports: HashMap, HashSet, SynchronizedRef, Tuple consolidated from Plan 01
- Typecheck passes
</success_criteria>
