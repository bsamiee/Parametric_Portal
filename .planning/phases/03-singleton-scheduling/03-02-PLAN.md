...
must_haves:
  ...
  key_links:
    - from: "packages/server/src/infra/cluster.ts"
      to: "@effect/platform"
      via: "KeyValueStore import for state persistence"
      pattern: "import.*KeyValueStore.*@effect/platform"
    - from: "packages/server/src/infra/cluster.ts"
...
**Type signature fix**: The `State` type parameter must be constrained to the schema's output type. Use conditional approach based on whether state is provided:
...
static readonly singleton = <E, R, StateSchema extends S.Schema.Any = never>(
  ...
  // Use ternary for cleaner conditional - Effect.if also valid but ternary clearer here
