...
must_haves:
  truths:
    - "SingletonError exists with 5 typed reasons (StateLoadFailed, StatePersistFailed, SchemaDecodeFailed, HeartbeatFailed, LeaderHandoffFailed)"
    - "MetricsService.singleton namespace contains duration, executions, lastExecution, stateErrors metrics"
    - "SQL-backed KeyValueStore layer exists for singleton state persistence"
  artifacts:
