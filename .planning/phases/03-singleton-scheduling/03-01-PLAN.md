...
must_haves:
  truths:
    ...
    - "KeyValueStore.modify is NOT atomic — critical sections require SqlClient.withTransaction wrapper"
  artifacts:
...
**CRITICAL**: Metrics must be module-level constants to prevent duplicate registration (research GROUP3 finding). The gauge is pre-created and reused across all singleton invocations.
...
// Consolidated effect import (Array as A, Option added to Task 1 imports):
...
- All imports consolidated: Data, Match, Number as N, Option, Array as A added to effect import
