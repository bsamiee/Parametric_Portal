...
Output:
- EventOutbox model with id, app_id, event_id, event_type, payload, status, published_at, created_at
...
NOTE: The migration already has these additions based on the research. Verify they exist and are correct. The migration should have:
...
class EventOutbox extends Model.Class<EventOutbox>('EventOutbox')({
  ...
  eventType: S.String,                              // Event variant tag: 'order', 'user', 'payment', 'system'
...
<task type="auto">
  <name>Task 3: Create eventOutbox repository</name>
  <files>packages/database/src/repos.ts</files>
  <action>
Add eventOutbox repository to repos.ts:
...
NOTE: The unified DLQ insert may need adjustment - check if jobDlq already has the insert method that can handle source field.
  </action>
  <verify>
...
2. Check that DatabaseService exports eventOutbox property
  ...
  <done>eventOutbox repository with offer, listPending, markPublished methods ready for EventBus</done>
</task>
...
5. DatabaseService.eventOutbox available with offer, listPending, markPublished methods
</verification>
...
- Repository supports offer (insert pending), listPending, markPublished, markFailed
