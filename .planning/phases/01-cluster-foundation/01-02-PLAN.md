...
must_haves:
  truths:
    ...
    - "Entity handlers implement idempotent pattern via SqlMessageStorage.saveRequest deduplication"
  artifacts:
  ...
  key_links:
    ...
    - from: "packages/server/src/infra/cluster.ts (Entity handler)"
      to: "@effect/cluster MessageStorage"
      via: "saveRequest for idempotency deduplication"
      pattern: "storage\\.saveRequest"
---
...
**Add to imports:**
```typescript
import { Entity, Rpc, Sharding, SqlMessageStorage, SqlRunnerStorage, NodeClusterSocket, ShardingConfig, MessageStorage } from '@effect/cluster';
...
const ClusterEntityLive = ClusterEntity.toLayer(Effect.gen(function* () {
  ...
  return {
    process: ({ data, idempotencyKey }) => Effect.gen(function* () {
      // Idempotency check via SqlMessageStorage.saveRequest (Success Criterion 6)
      // Returns 'Duplicate' if request already processed, 'New' if first time
      if (idempotencyKey) {
        const dedupeResult = yield* storage.saveRequest(idempotencyKey);
        if (dedupeResult._tag === 'Duplicate') {
          yield* Effect.logDebug('Duplicate request, skipping', { idempotencyKey });
          return;
        }
      }
...
- Idempotency via `storage.saveRequest(idempotencyKey)` - Success Criterion 6
  </action>
  <verify>
```bash
...
```
Check that saveRequest is used for idempotency:
```bash
grep -n "saveRequest" packages/server/src/infra/cluster.ts
```
  </verify>
  <done>
...
- Idempotent handler via MessageStorage.saveRequest deduplication
  </done>
</task>
...
3. **Idempotency pattern (Success Criterion 6):**
   ```bash
   grep -n "saveRequest" packages/server/src/infra/cluster.ts
   grep -n "Duplicate" packages/server/src/infra/cluster.ts
   ```
...
- [ ] Entity handler uses saveRequest for idempotency (Success Criterion 6)
