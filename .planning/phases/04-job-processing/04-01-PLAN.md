...
must_haves:
  truths:
    - "JobDlq model exists with all required fields for dead-letter functionality"
    - "jobDlq repo provides get, put, replay, byApp, byType methods"
    - "Migration creates job_dlq table with proper indexes"
  artifacts:
...
<task type="auto">
  ...
  <action>
Add JobDlq model after the Job model section. Follow existing Model.Class patterns.
...
Section comment: `// --- [JOBS: JOB_DLQ] ----------------------------------------------------------`
  ...
  <done>JobDlq model exists with all fields, properly exported</done>
</task>
...
- No changes to existing Job model or repo
</success_criteria>
