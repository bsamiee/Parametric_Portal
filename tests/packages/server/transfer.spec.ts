...
import { Array as A, Arbitrary, Chunk, Effect, Either, FastCheck as fc, Schema as S, Stream } from 'effect';
...
it.effect.prop('P8: error schemas', { fatal: _fatalError, parse: _parseError }, ({ parse, fatal }) => Effect.sync(() => {
    ...
    expect(parse.ordinal === undefined || typeof parse.ordinal === 'number').toBe(true);
...
class ImportCommand implements fc.AsyncCommand<Model, Real> {
    readonly format: Fmt;
    readonly items: readonly { content: string; type: string }[];
    constructor(format: Fmt, items: readonly { content: string; type: string }[]) { this.format = format; this.items = items; }
    check = () => true;
