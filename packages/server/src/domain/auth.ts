	...
	effect: Effect.gen(function* () {
		...
		const session = {
			lookup: (hash: Hex64) => Effect.gen(function* () {
				...
				yield* Option.liftPredicate(now, (n) => n <= sessionRow.expiryAccess.getTime());
			...
			verify: (sessionId: string, code: string, mode: 'backup' | 'totp') => Effect.gen(function* () {
				...
				const remainingCodes = yield* isBackup
					? Effect.forEach(mfaSecret.backups, Effect.fn(function* (entry, index) {
						...
						onSome: (index) => db.mfaSecrets.upsert({ backups: [...mfaSecret.backups.slice(0, index), ...mfaSecret.backups.slice(index + 1)], enabledAt: mfaSecret.enabledAt, encrypted: mfaSecret.encrypted, userId: sessionLoaded.userId }).pipe(Effect.tap(replayGuard.recordSuccess(sessionLoaded.userId)), Effect.as(mfaSecret.backups.length - 1), Effect.mapError(AuthError.from.bind(null, 'mfa_invalid_backup', { remaining: mfaSecret.backups.length, userId: sessionLoaded.userId }))),
					})))
