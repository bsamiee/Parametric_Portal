...
import { Cache as EffectCache, Duration, Effect, Metric, Option } from 'effect';
	...
	Effect.flatMap(
		...
		Option.match({
			...
			onSome: (m) => MetricsService.inc(
				type === 'hit' ? m.cache.hits : type === 'miss' ? m.cache.misses : m.cache.evictions,
...
const make = <K, V, E, R>(config: {
	...
	Effect.gen(function* () {
		...
		const _compositeKey = (key: K) => Effect.map(Context.Request.tenantId, (tenantId): CompositeKey => ({ key, tenantId }));
		...
		const refresh = (key: K): Effect.Effect<void, E, R> =>
			Effect.flatMap(_compositeKey(key), (ck) => internal.refresh(ck));

		const invalidate = (key: K): Effect.Effect<void, never, R> =>
			Effect.flatMap(_compositeKey(key), (ck) => internal.invalidate(ck));
		...
		const contains = (key: K): Effect.Effect<boolean, never, R> =>
			Effect.flatMap(_compositeKey(key), (ck) => internal.contains(ck));

		const stats = Effect.map(internal.cacheStats, (s) => ({ hits: s.hits, misses: s.misses, size: s.size }));
