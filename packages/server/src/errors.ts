...
class Auth extends S.TaggedError<Auth>()('Auth', { cause: S.optional(S.Unknown), details: S.String }, HttpApiSchema.annotations({ description: 'Authentication required', status: 401 })) {
    static readonly of = (details: string, cause?: unknown) => new Auth({ cause, details });
    override get message() { return `Auth: ${this.details}`; }}

class Conflict extends S.TaggedError<Conflict>()('Conflict', { cause: S.optional(S.Unknown), details: S.String, resource: S.String }, HttpApiSchema.annotations({ description: 'Resource conflict', status: 409 })) {
...
class Forbidden extends S.TaggedError<Forbidden>()('Forbidden', { cause: S.optional(S.Unknown), details: S.String }, HttpApiSchema.annotations({ description: 'Access denied', status: 403 })) {
...
class GatewayTimeout extends S.TaggedError<GatewayTimeout>()('GatewayTimeout', { cause: S.optional(S.Unknown), durationMs: S.Number, upstream: S.String }, HttpApiSchema.annotations({ description: 'Upstream timeout', status: 504 })) {
...
class Gone extends S.TaggedError<Gone>()('Gone', { cause: S.optional(S.Unknown), id: S.String, resource: S.String }, HttpApiSchema.annotations({ description: 'Resource gone', status: 410 })) {
...
class Internal extends S.TaggedError<Internal>()('Internal', { cause: S.optional(S.Unknown), details: S.String }, HttpApiSchema.annotations({ description: 'Internal server error', status: 500 })) {
...
class NotFound extends S.TaggedError<NotFound>()('NotFound', { cause: S.optional(S.Unknown), id: S.optional(S.String), resource: S.String }, HttpApiSchema.annotations({ description: 'Resource not found', status: 404 })) {
...
class OAuth extends S.TaggedError<OAuth>()('OAuth', { cause: S.optional(S.Unknown), details: S.String, provider: S.String }, HttpApiSchema.annotations({ description: 'OAuth provider error', status: 400 })) {
...
class RateLimit extends S.TaggedError<RateLimit>()('RateLimit', { cause: S.optional(S.Unknown), limit: S.optional(S.Number), recoveryAction: S.optional(S.Literal('email-verify', 'support-ticket')), remaining: S.optional(S.Number), resetAfterMs: S.optional(S.Number), retryAfterMs: S.Number }, HttpApiSchema.annotations({ description: 'Rate limit exceeded', status: 429 })) {
...
class ServiceUnavailable extends S.TaggedError<ServiceUnavailable>()('ServiceUnavailable', { cause: S.optional(S.Unknown), details: S.String, retryAfterMs: S.Number }, HttpApiSchema.annotations({ description: 'Service unavailable', status: 503 })) {
...
class Validation extends S.TaggedError<Validation>()('Validation', { cause: S.optional(S.Unknown), details: S.String, field: S.String }, HttpApiSchema.annotations({ description: 'Validation failed', status: 400 })) {
