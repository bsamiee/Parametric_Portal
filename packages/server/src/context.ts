...
import { Config, Data, Duration, Effect, FiberId, FiberRef, Layer, Option, Order, pipe, Record, Schedule, Schema as S } from 'effect';
	...
	static readonly fromData = (ctx: Context.Request.Data): Serializable =>
		new Serializable({
			...
			...Option.match(ctx.cluster, {	// Cluster fields: null → undefined for S.optional, ShardId → branded string
				onNone: constant({}),
				onSome: (cluster) => ({ runnerId: cluster.runnerId ?? undefined, shardId: cluster.shardId ? Serializable.makeShardIdString(cluster.shardId) : undefined }),
			}),
		});
}
...
class Request extends Effect.Tag('server/RequestContext')<Request, Context.Request.Data>() {
	...
	static readonly toAttrs = (ctx: Context.Request.Data, fiberId: FiberId.FiberId): Record.ReadonlyRecord<string, string> =>	// Observability attributes for tracing spans. Includes all context fields formatted for OTEL.
		Record.getSomes({
			...
			'circuit.name': 			Option.map(ctx.circuit, (circuit) => circuit.name),
			'circuit.state': 			Option.map(ctx.circuit, (circuit) => circuit.state),
			'client.ip': 				ctx.ipAddress,
			'client.ua': 				Option.map(ctx.userAgent, (userAgent) => (userAgent.length > 120 ? `${userAgent.slice(0, 117)}...` : userAgent)),
			'cluster.entity_id': 		Option.flatMapNullable(ctx.cluster, (cluster) => cluster.entityId),
			'cluster.entity_type': 		Option.flatMapNullable(ctx.cluster, (cluster) => cluster.entityType),
			'cluster.is_leader': 		Option.map(ctx.cluster, (cluster) => String(cluster.isLeader)),
			'cluster.runner_id':		Option.flatMapNullable(ctx.cluster, (cluster) => cluster.runnerId),
			...
			'ratelimit.delay_ms':		Option.map(ctx.rateLimit, (rateLimit) => String(Duration.toMillis(rateLimit.delay))),
			'ratelimit.limit': 			Option.map(ctx.rateLimit, (rateLimit) => String(rateLimit.limit)),
			'ratelimit.remaining': 		Option.map(ctx.rateLimit, (rateLimit) => String(rateLimit.remaining)),
				'ratelimit.reset_after_ms': Option.map(ctx.rateLimit, (rateLimit) => String(Duration.toMillis(rateLimit.resetAfter))),
				'request.id': 				Option.some(ctx.requestId),
				'session.kind': 			Option.map(ctx.session, (session) => session.kind),
				'session.mfa': 				Option.map(ctx.session, (session) => String(session.mfaEnabled)),
	...
	static readonly auditFields = Request.current.pipe(Effect.map((ctx): Context.Request.AuditFields => ({
		ipAddress: Option.getOrUndefined(ctx.ipAddress),
		requestId: ctx.requestId,
		userAgent: Option.getOrUndefined(ctx.userAgent),
	})));
	...
	static readonly shardId = Request.current.pipe(Effect.map((ctx) => Option.flatMapNullable(ctx.cluster, (cluster) => cluster.shardId)));
	static readonly runnerId = Request.current.pipe(Effect.map((ctx) => Option.flatMapNullable(ctx.cluster, (cluster) => cluster.runnerId)));
	static readonly isLeader = Request.current.pipe(Effect.map((ctx) => Option.exists(ctx.cluster, (cluster) => cluster.isLeader)));
