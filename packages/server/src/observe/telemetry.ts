...
const _causeAttrs = (cause: Cause.Cause<unknown>): Record<string, unknown> => {
	...
	const msg = pipe(pretty, Option.map((entry) => entry.message), Option.getOrElse(() => Cause.pretty(cause)));
			...
			Effect.flatMap(([ctx, fiberId]) => {
				...
				return opts?.metrics === false
					...
					: Effect.flatMap(Effect.serviceOption(MetricsService), Option.match({
							onNone: () => coreSpan,
							onSome: (metrics) => MetricsService.trackEffect(coreSpan, { duration: metrics.http.duration, errors: metrics.errors, labels: MetricsService.label({ operation: name, tenant: ctx.tenantId }) }),
						}));
			}),
...
const _Default = Layer.unwrapEffect(
	_TELEMETRY_CONFIG.pipe(Effect.map((configuration) => {
		...
		const baseLogger = configuration.environment === 'production' ? Logger.jsonLogger : Logger.prettyLogger({ colors: 'auto', mode: 'auto' });
...
const Telemetry = { config: _CONFIG, Default: _Default, span: _span } as const;
