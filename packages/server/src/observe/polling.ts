...
const _CONFIG = {
	...
	refresh: { minInterval: Duration.seconds(15), tenantMetric: { concurrency: 4, timeout: Duration.seconds(5) } },
	thresholds: {
...
class PollingService extends Effect.Service<PollingService>()('server/Polling', {
	scoped: Effect.gen(function* () {
		...
		const publishFailure = (metric: string, error: unknown) =>
			eventBus.publish({ aggregateId: metric, payload: { _tag: 'polling.error', error: String(error), metric }, tenantId: Context.Request.Id.system }).pipe(Effect.ignore);
		const recoverWith = <A>(metric: string, message: string, fallback: Effect.Effect<A>) => (error: unknown) =>
			Effect.all([Effect.logError(message, { error: String(error), metric }), publishFailure(metric, error)], { discard: true }).pipe(Effect.andThen(fallback));
		...
		const refresh = (force = false) => Effect.gen(function* () {
			...
			const elapsed = now - (Option.getOrUndefined(yield* STM.commit(TRef.get(lastRefreshAtMs))) ?? _CONFIG.fallback.metric);
			yield* Effect.when(Effect.all([pollDlqSize, pollJobQueueDepth, pollEventOutboxDepth, pollIoStats], { discard: true }).pipe(Effect.andThen(STM.commit(TRef.set(lastRefreshAtMs, Option.some(now))))), () => force || elapsed >= Duration.toMillis(_CONFIG.refresh.minInterval));
		}).pipe(Telemetry.span('polling.refresh', { metrics: false }));
		const getHealth = () => _loadAlerts.pipe(
