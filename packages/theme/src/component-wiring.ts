...
const ComponentSpecSchema = S.Struct({
    asyncStyles: S.optional(
        S.Record({
            key: AsyncStyleKeySchema,
...
const selectorFor: Record<RuleKind, (name: string, qualifier?: string) => string> = {
    async: (n, q) =>
        q === 'loading'
            ...
            : `[data-slot="${n}"][data-async-state="${q}"]`,
    base: (n) => `[data-slot="${n}"]`,
...
const generateSingleComponentWiring = (spec: ComponentSpec, colorNames: readonly string[]): string =>
    [
        ...
        ...(spec.variants === undefined
            ? []
            : Object.entries(spec.variants).map(([variant, values]) =>
                  generateRule(spec.name, 'variant', values, variant),
              )),
        ...(spec.asyncStyles === undefined
            ? []
            : Object.entries(spec.asyncStyles).map(([state, values]) =>
                  generateRule(spec.name, 'async', values, state),
              )),
    ].join('\n\n');
