...
export default Effect.gen(function* () {
    ...
    yield* sql.unsafe(String.raw`DO $$ DECLARE _ext text; _kv record; BEGIN
        ...
        FOR _kv IN SELECT * FROM (VALUES
            ...
            ('pg_prewarm.autoprewarm','on'),('pg_prewarm.autoprewarm_interval','300')
        ) AS t(name, val) LOOP
    ...
    yield* sql.unsafe(String.raw`
        ...
        CREATE OR REPLACE FUNCTION _register_monthly_partition(
            p_table text, p_control text, p_retention text,
        ...
        BEGIN
            PERFORM partman.create_parent(
                ...
                p_control := p_control, p_type := 'range',
        ...
        END $fn$;
        CREATE INDEX idx_sessions_app_user_active
            ON sessions(app_id, user_id)
            INCLUDE (expiry_access, expiry_refresh, verified_at, updated_at, ip_address)
    ...
    yield* sql.unsafe(String.raw`
        ...
        CREATE UNIQUE INDEX idx_notifications_dedupe_active
            ON notifications(app_id, (correlation->>'dedupe'))
            WHERE correlation->>'dedupe' IS NOT NULL
    ...
    yield* sql.unsafe(String.raw`
        ...
        DECLARE
            _cutoff TIMESTAMPTZ := NOW() - make_interval(days => p_older_than_days);
            _deleted INT := 0; _dropped INT := 0;
            _rel record; _upper TEXT;
        BEGIN
            FOR _rel IN
                ...
                WHERE inh.inhparent = 'public.sessions'::regclass
            LOOP
                IF _rel.bound <> 'DEFAULT' THEN
                    _upper := (regexp_match(
                        _rel.bound, $$TO \('([^']+)'\)$$))[1];
                    IF _upper IS NOT NULL
                        AND _upper::timestamptz < _cutoff
                    THEN
                        EXECUTE format('DROP TABLE IF EXISTS %I.%I',
                            _rel.nspname, _rel.relname);
                        _dropped := _dropped + 1;
                    END IF;
                END IF;
            END LOOP;
    ...
    DO $$ DECLARE _rec record; BEGIN
        FOR _rec IN SELECT * FROM (VALUES
            ...
            EXECUTE format($fn$
                CREATE OR REPLACE FUNCTION purge_%I(
                    p_older_than_days INT DEFAULT %s
                ) RETURNS INT LANGUAGE sql VOLATILE AS $body$
                    WITH purged AS (
                        DELETE FROM %I
                        WHERE %I IS NOT NULL
                            AND %I < NOW() - make_interval(
                                days => p_older_than_days)
                        RETURNING 1
    ...
    yield* sql.unsafe(String.raw`
        ...
        CREATE OR REPLACE FUNCTION revoke_sessions_by_ip(
            p_app_id UUID, p_ip INET
    ...
    yield* sql.unsafe(String.raw`
        CREATE OR REPLACE FUNCTION stat(
            p_name TEXT, p_limit INT DEFAULT 100,
            p_extra JSONB DEFAULT NULL
        ) RETURNS JSONB LANGUAGE plpgsql STABLE AS $fn$
        DECLARE
            _lim TEXT := GREATEST(1, LEAST(p_limit, 500))::text;
            _result JSONB; _sql TEXT;
        BEGIN _sql := CASE p_name
            WHEN 'io_config' THEN
                ...
                ']) ORDER BY name'
            WHEN 'cache_ratio' THEN
                'SELECT backend_type,'
                ' object AS io_object,'
                ' context AS io_context,'
                ' SUM(hits)::float8 AS hits,'
                ' SUM(reads)::float8 AS reads,'
                ' SUM(writes)::float8 AS writes,'
                ' CASE WHEN SUM(hits)+SUM(reads)>0'
                '   THEN SUM(hits)::float8'
                '       /(SUM(hits)::float8+SUM(reads)::float8)*100'
            ...
            WHEN 'io_detail' THEN
                'SELECT backend_type,'
                ' object AS io_object, context AS io_context,'
                ' reads, read_time, read_bytes,'
                ' writes, write_time, write_bytes,'
                ' writebacks, writeback_time,'
                ' extends, extend_time, extend_bytes,'
                ' hits, evictions, reuses,'
                ' fsyncs, fsync_time, stats_reset'
                ' FROM pg_stat_io'
            WHEN 'statements' THEN
                'WITH info AS ('
                '  SELECT dealloc::float8 AS dealloc,'
                '    stats_reset'
                '  FROM pg_stat_statements_info'
                ') SELECT s.queryid::float8,'
                ' s.userid::float8, s.dbid::float8,'
                ' s.toplevel, s.calls::float8,'
                ' s.plans::float8,'
                ' s.total_plan_time, s.mean_plan_time,'
                ' s.total_exec_time, s.mean_exec_time,'
                ' s.rows::float8,'
                ' s.shared_blks_hit::float8,'
                ' s.shared_blks_read::float8,'
                ' s.shared_blks_dirtied::float8,'
                ' s.shared_blks_written::float8,'
                ' s.temp_blks_read::float8,'
                ' s.temp_blks_written::float8,'
                ' s.blk_read_time, s.blk_write_time,'
                ' s.wal_records::float8,'
                ' s.wal_fpi::float8,'
                ' s.wal_bytes::float8,'
                ' s.parallel_workers_to_launch::float8,'
                ' s.parallel_workers_launched::float8,'
                ' s.wal_buffers_full::float8,'
                ' i.dealloc, i.stats_reset, s.query'
                ' FROM pg_stat_statements s'
                ' CROSS JOIN info i'
                ' ORDER BY s.total_exec_time'
                '   DESC NULLS LAST LIMIT ' || _lim
            WHEN 'wal_inspect' THEN
                'SELECT * FROM pg_get_wal_record_info('
                '  pg_current_wal_lsn()) LIMIT ' || _lim
            WHEN 'kcache' THEN
                'SELECT k.queryid::float8,'
                ' d.datname, r.rolname, k.top,'
                ' s.calls::float8,'
                ' s.total_exec_time, s.mean_exec_time,'
                ' k.plan_user_time, k.plan_system_time,'
                ' k.plan_reads::float8,'
                ' k.plan_writes::float8,'
                ' k.exec_user_time, k.exec_system_time,'
                ' k.exec_reads::float8,'
                ' k.exec_writes::float8,'
                ' CASE WHEN s.calls > 0'
                '   THEN k.exec_reads::float8'
                '       / s.calls::float8'
                '   END AS reads_per_call,'
                ' CASE WHEN s.calls > 0'
                '   THEN k.exec_writes::float8'
                '       / s.calls::float8'
                '   END AS writes_per_call,'
                ' k.stats_since, s.query'
                ' FROM pg_stat_kcache() k'
                ' JOIN pg_stat_statements s'
                '   ON s.queryid=k.queryid'
                '   AND s.userid=k.userid'
                '   AND s.dbid=k.dbid'
                ' JOIN pg_database d ON d.oid=k.dbid'
                ' JOIN pg_roles r ON r.oid=k.userid'
                ' ORDER BY k.exec_reads'
                '   DESC NULLS LAST LIMIT ' || _lim
            WHEN 'qualstats' THEN
                'WITH advisor_queryids AS ('
                '  SELECT DISTINCT'
                '    (qid.value)::text::float8 AS queryid'
                '  FROM jsonb_array_elements('
                '    COALESCE(('
                '      pg_qualstats_index_advisor('
                '        min_filter => 1000,'
                '        min_selectivity => 30,'
                '        forbidden_am => ARRAY[''hash'']'
                '      ))::jsonb->''indexes'','
                '      ''[]''::jsonb'
                '    )) idx(item)'
                '  CROSS JOIN LATERAL jsonb_array_elements('
                '    COALESCE(idx.item->''queryids'','
                '      ''[]''::jsonb)) qid(value)'
                '  WHERE jsonb_typeof(qid.value)=''number'''
                '), ranked AS ('
                '  SELECT bq.queryid::float8,'
                '    bq.uniquequalnodeid::float8,'
                '    bq.qualnodeid::float8,'
                '    bq.userid::float8, bq.dbid::float8,'
                '    bq.occurences::float8,'
                '    bq.execution_count::float8,'
                '    bq.nbfiltered::float8,'
                '    CASE WHEN bq.execution_count > 0'
                '      THEN (bq.nbfiltered::float8'
                '        / bq.execution_count::float8) * 100'
                '      ELSE 0 END AS filter_ratio_pct,'
                '    bq.constvalues::text[],'
                '    to_jsonb(bq.quals) AS quals,'
                '    pg_qualstats_example_query('
                '      bq.queryid::bigint) AS example_query,'
                '    CASE WHEN aq.queryid IS NULL'
                '      THEN 0 ELSE 1'
                '      END AS advisor_rank'
                '  FROM pg_qualstats_by_query bq'
                '  LEFT JOIN advisor_queryids aq'
                '    ON aq.queryid = bq.queryid::float8'
                ') SELECT queryid, uniquequalnodeid,'
                '  qualnodeid, userid, dbid,'
                '  occurences, execution_count,'
                '  nbfiltered, filter_ratio_pct,'
                '  constvalues, quals, example_query'
                ' FROM ranked'
                ' ORDER BY advisor_rank DESC,'
                '  execution_count DESC NULLS LAST,'
                '  nbfiltered DESC NULLS LAST'
                ' LIMIT ' || _lim
            WHEN 'index_advisor' THEN
                'SELECT idx.item->>''index'' AS index_ddl,'
                '  idx.item->''queryids'' AS queryids,'
                '  idx.item->>''am'' AS access_method'
                ' FROM jsonb_array_elements('
                '  COALESCE(('
                '    pg_qualstats_index_advisor('
                '      min_filter => '
                    || COALESCE(
                        (p_extra->>'min_filter')::int,
                        1000)::text
                    || ', min_selectivity => '
                    || COALESCE(
                        (p_extra->>'min_selectivity')::int,
                        30)::text
                    || ', forbidden_am => ARRAY[''hash'']'
                '    ))::jsonb->''indexes'','
                '    ''[]''::jsonb'
                '  )) idx(item)'
            WHEN 'wait_sampling' THEN
                ...
                '  DESC NULLS LAST LIMIT ' || _lim
            WHEN 'wait_sampling_current' THEN
                'SELECT pid::bigint, event_type,'
                '  event, queryid::float8'
                ' FROM pg_wait_sampling_current'
                ' ORDER BY pid DESC LIMIT ' || _lim
            WHEN 'wait_sampling_history' THEN
                'SELECT ts AS sample_ts,'
                '  pid::bigint, event_type,'
                '  event, queryid::float8'
                ' FROM pg_wait_sampling_history'
                ' WHERE ts >= now() - make_interval('
                '  secs => GREATEST(1, LEAST('
                    || COALESCE(
                        (p_extra->>'since_seconds')::int,
                        60)::text
                    || ', 3600)))'
                ' ORDER BY ts DESC LIMIT ' || _lim
            WHEN 'buffercache_summary' THEN
                'SELECT buffers_used::bigint,'
                '  buffers_unused::bigint,'
                '  buffers_dirty::bigint,'
                '  buffers_pinned::bigint,'
                '  usagecount_avg::float8'
                ' FROM pg_buffercache_summary()'
            WHEN 'buffercache_usage' THEN
                'SELECT usage_count::int,'
                '  buffers::bigint, dirty::bigint,'
                '  pinned::bigint'
                ' FROM pg_buffercache_usage_counts()'
                ' ORDER BY usage_count'
            WHEN 'buffercache_top' THEN
                'SELECT c.relname::text,'
                '  c.relkind::text,'
                '  count(*)::bigint AS buffers,'
                '  pg_size_pretty(count(*)::bigint'
                '    * current_setting(''block_size'')::bigint'
                '  ) AS size,'
                '  round(100.0 * count(*)'
                '    / NULLIF((SELECT buffers_used'
                '      FROM pg_buffercache_summary()),0)'
                '    ,2)::float8 AS pct'
            ...
            WHEN 'visibility' THEN
                'SELECT c.relname::text,'
                '  c.relkind::text,'
                '  v.all_visible::bigint,'
                '  v.all_frozen::bigint,'
                '  pg_relation_size(c.oid)::bigint'
                '    AS rel_size'
                ' FROM pg_class c'
                ' CROSS JOIN LATERAL'
                '  pg_visibility_map_summary(c.oid) v'
                ' WHERE c.relkind IN (''r'',''m'')'
                '  AND c.relnamespace'
                '    = ''public''::regnamespace'
                ' ORDER BY pg_relation_size(c.oid) DESC'
                ' LIMIT ' || _lim
            WHEN 'hypothetical_indexes' THEN
                'SELECT indexrelid::bigint, indexname,'
                '  nspname, relname, amname'
                ' FROM hypopg_list_indexes'
            WHEN 'cron_history' THEN
                'SELECT d.runid::bigint, j.jobname,'
                '  d.job_pid::bigint, d.database,'
                '  d.username, d.command, d.status,'
                '  d.return_message,'
                ...
                ' WHERE ('
                    || COALESCE(
                        quote_literal(p_extra->>'job_name'),
                        'NULL')
                    || ' IS NULL OR j.jobname = '
                    || COALESCE(
                        quote_literal(p_extra->>'job_name'),
                        'NULL')
                    || ')'
                ' ORDER BY d.runid DESC LIMIT ' || _lim
            WHEN 'cron_failures' THEN
                'SELECT d.runid::bigint, j.jobname,'
            ...
            WHEN 'partman_config' THEN
                'SELECT parent_table, control,'
                '  partition_interval, premake,'
                '  retention, infinite_time_partitions'
                ' FROM partman.part_config'
                ' ORDER BY parent_table'
            WHEN 'squeeze_tables' THEN
                'SELECT relation::text, schedule,'
                '  free_space_extra, vacuum_max_age,'
                '  max_retry, active'
                ' FROM squeeze.tables'
                ' ORDER BY relation::text'
            WHEN 'squeeze_workers' THEN
                'SELECT pid::int'
                ' FROM squeeze.get_active_workers() pid'
                ' ORDER BY pid'
            ...
            WHEN 'table_bloat' THEN
                ...
                '   pg_total_relation_size('
                '   (schemaname||''.''||tablename'
                '   )::regclass)) AS total_size,'
                ' pg_size_pretty('
                '   pg_table_size('
                '   (schemaname||''.''||tablename'
                '   )::regclass)) AS table_size'
                ' FROM pg_tables'
                ...
                '   )::regclass)'
                '   DESC LIMIT ' || _lim
            WHEN 'index_bloat' THEN
                'SELECT schemaname, tablename,'
                ' indexname,'
                ' pg_relation_size(indexrelid)::bigint'
                '   AS index_bytes,'
                ' pg_size_pretty('
                ...
                '   DESC NULLS LAST LIMIT ' || _lim
            WHEN 'lock_contention' THEN
                'SELECT blocked.pid::bigint'
                '   AS blocked_pid,'
                ...
                '   AND blocking.database'
                '     IS NOT DISTINCT FROM blocked.database'
                '   AND blocking.relation'
                '     IS NOT DISTINCT FROM blocked.relation'
                '   AND blocking.page'
                '     IS NOT DISTINCT FROM blocked.page'
                '   AND blocking.tuple'
                '     IS NOT DISTINCT FROM blocked.tuple'
                '   AND blocking.virtualxid'
                '     IS NOT DISTINCT FROM'
                '       blocked.virtualxid'
                '   AND blocking.transactionid'
                '     IS NOT DISTINCT FROM'
                '       blocked.transactionid'
                '   AND blocking.classid'
                '     IS NOT DISTINCT FROM blocked.classid'
                '   AND blocking.objid'
                '     IS NOT DISTINCT FROM blocked.objid'
                '   AND blocking.objsubid'
                '     IS NOT DISTINCT FROM blocked.objsubid'
                ...
                '   DESC NULLS LAST LIMIT ' || _lim
            WHEN 'long_running_queries' THEN
                'SELECT pid::bigint, usename,'
                ' datname, state,'
                ' wait_event_type, wait_event,'
                ' query,'
                ' now() - query_start AS duration,'
                ' EXTRACT(EPOCH FROM'
                '   now() - query_start)::float8'
                '   AS duration_seconds,'
                ...
                '   AND query_start < now()'
                '     - make_interval(secs => GREATEST(1,'
                '       LEAST('
                    || COALESCE(
                        (p_extra->>'min_seconds')::int,
                        5)::text
                    || ', 3600)))'
                ' ORDER BY query_start ASC'
                ' LIMIT ' || _lim
            WHEN 'connection_stats' THEN
                'SELECT datname, usename,'
                ' client_addr::text,'
                ' state, count(*)::bigint AS cnt,'
                ...
                '   client_addr, state'
                ' ORDER BY cnt DESC LIMIT ' || _lim
            WHEN 'replication_lag' THEN
            ...
            WHEN 'index_usage' THEN
                ...
                '   pg_relation_size(indexrelid)'
                '   DESC NULLS LAST LIMIT ' || _lim
            WHEN 'table_sizes' THEN
                'SELECT schemaname, relname,'
                ' n_live_tup::bigint,'
                ' n_dead_tup::bigint,'
                ' pg_total_relation_size('
                '   relid)::bigint AS total_bytes,'
                ' pg_table_size('
                '   relid)::bigint AS table_bytes,'
                ' pg_indexes_size('
                '   relid)::bigint AS index_bytes,'
                ' pg_size_pretty('
            ...
            WHEN 'seq_scan_heavy' THEN
                ...
                ' WHERE seq_scan > 0'
                '   AND n_live_tup > 1000'
                ' ORDER BY seq_scan DESC LIMIT ' || _lim
            ELSE NULL END;
        ...
        CREATE OR REPLACE FUNCTION prewarm_relation(p_relation TEXT, p_mode TEXT DEFAULT 'buffer')
            RETURNS INT LANGUAGE sql VOLATILE AS $$ SELECT pg_prewarm(p_relation::regclass, p_mode) $$;
        CREATE OR REPLACE FUNCTION heap_force_freeze(p_relation TEXT, p_block BIGINT DEFAULT 0)
            RETURNS void LANGUAGE sql VOLATILE
            AS $$ SELECT pg_surgery.heap_force_freeze(p_relation::regclass, ARRAY[ROW(p_block,0)::tid]) $$;
        CREATE OR REPLACE FUNCTION create_hypothetical_index(p_statement TEXT)
            RETURNS TABLE(indexrelid bigint, indexname text) LANGUAGE sql VOLATILE
            AS $$ SELECT * FROM hypopg_create_index(p_statement) $$;
    ...
    DO $$ DECLARE _rec record; BEGIN
        ...
        END LOOP;
    END $$;
        CREATE OR REPLACE FUNCTION stop_squeeze_worker(p_pid INT) RETURNS BOOLEAN LANGUAGE sql VOLATILE AS $$ SELECT squeeze.stop_worker(p_pid) IS NOT NULL $$;
        CREATE OR REPLACE FUNCTION list_partition_health(p_parent_table TEXT) RETURNS JSONB LANGUAGE sql STABLE AS $$
            ...
            FROM pg_partition_tree(p_parent_table::regclass) tree
        ...
        BEGIN
            FOR _job IN SELECT * FROM (VALUES
                ...
                ('maintenance-vacuum-jobs','0 4 * * *',
                    'VACUUM (BUFFER_USAGE_LIMIT ''256MB'', ANALYZE) jobs'),
                ('maintenance-vacuum-sessions','10 4 * * *',
                    'VACUUM (BUFFER_USAGE_LIMIT ''256MB'', ANALYZE) sessions'),
                ('maintenance-vacuum-notifications','20 4 * * *',
                    'VACUUM (BUFFER_USAGE_LIMIT ''256MB'', ANALYZE) notifications'),
                ('maintenance-vacuum-audit-logs','30 4 * * *',
                    'VACUUM (BUFFER_USAGE_LIMIT ''256MB'', ANALYZE) audit_logs'),
                ('maintenance-reindex-sessions','0 5 * * 0',
                    'REINDEX INDEX CONCURRENTLY idx_sessions_app_user_active'),
                ('maintenance-reindex-jobs','10 5 * * 0',
                    'REINDEX INDEX CONCURRENTLY idx_jobs_app_status'),
                ('maintenance-reindex-notifications','20 5 * * 0',
                    'REINDEX INDEX CONCURRENTLY idx_notifications_app_status'),
                ('maintenance-purge-cron-history','40 3 * * *',
                    'DELETE FROM cron.job_run_details WHERE end_time < now() - interval ''7 days'''),
                ('maintenance-stat-statements-reset','0 0 * * 1',
                    'SELECT pg_stat_statements_reset()')
            ) AS t(name, schedule, command) LOOP BEGIN
                SELECT j.jobid, j.schedule, j.command
                    INTO _existing_jobid, _existing_schedule, _existing_command
                    FROM cron.job j WHERE j.jobname = _job.name
    ...
    yield* sql.unsafe(String.raw`
        CREATE OR REPLACE FUNCTION refresh_search_documents(
            p_scope_id uuid DEFAULT NULL, p_include_global boolean DEFAULT false)
        ...
        BEGIN
            ...
            IF p_scope_id IS NULL OR p_include_global THEN INSERT INTO search_documents (entity_type, entity_id, scope_id, display_text, content_text, metadata, normalized_text)
                SELECT 'app', s.id, NULL, s.name, s.namespace,
                    ...
                    normalize_search_text(s.name, s.namespace,
                        jsonb_build_object('name',s.name,'namespace',s.namespace))
                    FROM apps s; END IF;
            INSERT INTO search_documents (entity_type, entity_id, scope_id, display_text, content_text, metadata, normalized_text)
            SELECT 'user', s.id, s.app_id, s.email, s.role::text,
                jsonb_build_object('email',s.email,'role',s.role),
                normalize_search_text(s.email, s.role::text,
                    jsonb_build_object('email',s.email,'role',s.role))
                FROM users s WHERE s.deleted_at IS NULL
                AND (p_scope_id IS NULL OR s.app_id = p_scope_id);
            INSERT INTO search_documents (entity_type, entity_id, scope_id, display_text, content_text, metadata, normalized_text)
            SELECT 'asset', s.id, s.app_id, COALESCE(s.name,s.type), s.content,
                jsonb_strip_nulls(jsonb_build_object(
                    'type',s.type,'size',s.size,'name',s.name,'hash',s.hash),true),
                normalize_search_text(COALESCE(s.name,s.type), s.content,
                    jsonb_strip_nulls(jsonb_build_object(
                        'type',s.type,'size',s.size,'name',s.name,'hash',s.hash),true))
                FROM assets s WHERE s.deleted_at IS NULL
                AND (p_scope_id IS NULL OR s.app_id = p_scope_id);
            INSERT INTO search_documents (entity_type, entity_id, scope_id, display_text, content_text, metadata, normalized_text)
            SELECT 'auditLog', s.id, s.app_id,
                s.target_type||':'||s.operation::text,
                s.target_type||' '||s.operation::text,
                jsonb_strip_nulls(jsonb_build_object(
                    'targetType',s.target_type,'operation',s.operation,
                    'userId',s.user_id,'hasDelta',s.delta IS NOT NULL),true),
                normalize_search_text(
                    ...
                    jsonb_strip_nulls(jsonb_build_object(
                        ...
                        'userId',s.user_id,'hasDelta',s.delta IS NOT NULL),true))
                FROM audit_logs s
                WHERE p_scope_id IS NULL OR s.app_id = p_scope_id;
        ...
        CREATE OR REPLACE FUNCTION get_search_suggestions(p_prefix text, p_scope_id uuid, p_include_global boolean, p_limit int DEFAULT 20)
            ...
            prefix_hits AS (
                ...
                FROM scoped_terms WHERE term LIKE (escaped_prefix || '%') ESCAPE '\'
                GROUP BY term ORDER BY frequency DESC, term ASC
            ...
            fuzzy_candidates AS (
                ...
                FROM scoped_terms WHERE char_length(prefix) >= 2
                    AND (term % prefix OR prefix <% term OR prefix <<% term)
                ORDER BY similarity_distance ASC, word_distance ASC,
                    strict_word_distance ASC, frequency DESC, term ASC
                LIMIT (SELECT max_limit * 4 FROM normalized)),
            fuzzy_hits AS (
                SELECT term, SUM(frequency)::bigint AS frequency
                FROM fuzzy_candidates WHERE lev <= 2
                    AND NOT EXISTS (SELECT 1 FROM prefix_hits p WHERE p.term = fuzzy_candidates.term)
                GROUP BY term
                ORDER BY MIN(lev) ASC, MIN(similarity_distance) ASC,
                    MIN(word_distance) ASC, MIN(strict_word_distance) ASC,
                    SUM(frequency) DESC, term ASC
                LIMIT (SELECT max_limit FROM normalized)),
    ...
    yield* sql.unsafe(String.raw`
        ...
        privileged_actions(resource, action) AS (VALUES ('users','updateRole'),('audit','getByEntity'),('audit','getByUser'),('search','refresh'),('search','refreshEmbeddings'),
            ...
            ('admin','deadTuples'),('admin','tableBloat'),('admin','indexBloat'),('admin','lockContention'),('admin','longRunningQueries'),('admin','connectionStats'),('admin','replicationLag'),('admin','indexUsage'),('admin','tableSizes'),('admin','unusedIndexes'),('admin','seqScanHeavy'),('admin','indexAdvisor'),('admin','hypotheticalIndexes'),('admin','createHypotheticalIndex'),('admin','resetHypotheticalIndexes'),('admin','visibility'),
    ...
    DO $$ DECLARE _r record; BEGIN
        FOR _r IN SELECT * FROM (VALUES
            ...
            EXECUTE format('CREATE POLICY %I ON %I %s', _r.tbl||'_tenant_isolation', _r.tbl, CASE _r.kind
                ...
                WHEN 'session' THEN 'USING (EXISTS (SELECT 1 FROM sessions s WHERE s.id = session_tokens.session_id AND s.app_id = get_current_tenant_id())) WITH CHECK (EXISTS (SELECT 1 FROM sessions s WHERE s.id = session_tokens.session_id AND s.app_id = get_current_tenant_id()))'
    ...
    yield* sql.unsafe(String.raw`DO $$ DECLARE _rec record; BEGIN
        FOR _rec IN SELECT * FROM (VALUES ('public.jobs','0 2 * * *',20,2),('public.kv_store','30 2 * * *',20,2),('public.session_tokens','0 3 * * *',20,3)) AS t(relation text, schedule text, free_space_extra int, max_retry int) LOOP
            INSERT INTO squeeze.tables (tabschema, tabname, schedule, free_space_extra, max_retry) VALUES (split_part(_rec.relation, '.', 1), split_part(_rec.relation, '.', 2), _rec.schedule::text, _rec.free_space_extra, _rec.max_retry) ON CONFLICT DO NOTHING;
        END LOOP;
    END $$`);
});
