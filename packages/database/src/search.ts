...
const _cfg = {
    docsTable: 'search_documents', embeddingsTable: 'search_embeddings',
    ...
    mrl: { profiles: { 'gemini:gemini-embedding-001:3072': 1024, 'openai:text-embedding-3-large:3072': 1024 } as Record<string, number> },
...
const _embeddingProfileSchema = S.Struct({ dimensions: S.Int.pipe(S.greaterThan(0), S.lessThanOrEqualTo(4_000)), model: S.NonEmptyTrimmedString, provider: S.Literal('anthropic', 'gemini', 'openai') });
...
const _embedRequestFields = { ..._embeddingProfileSchema.fields, ..._entityFilter, limit: S.Int.pipe(S.positive(), S.lessThanOrEqualTo(_cfg.limits.embedBatch)) };

// --- [FUNCTIONS] -------------------------------------------------------------

const _clamp  = (value: number | undefined, fallback: number, max: number) => Math.min(Math.max(value ?? fallback, 1), max);
const _mapErr = (operation: string) => Effect.mapError((cause: unknown) => cause instanceof SearchError ? cause : new SearchError({ cause, operation, reason: 'unknown' }));
const _spreadEmbedOpts = (options: _EmbedOpts) => ({
    dimensions: options.profile.dimensions, entityTypes: options.entityTypes ?? [], includeGlobal: options.includeGlobal ?? false,
    limit: _clamp(options.limit, _cfg.limits.embedBatch, _cfg.limits.embedBatch), model: options.profile.model, provider: options.profile.provider, scopeId: options.scopeId ?? null,
});
...
class SearchRepo extends Effect.Service<SearchRepo>()('database/Search', {
    effect: Effect.gen(function* () {
        ...
        const _distance = (alias: string, profile: EmbeddingProfile, embeddingJson: string): Statement.Fragment => {
            const dims = _cfg.mrl.profiles[`${profile.provider}:${profile.model}:${String(profile.dimensions)}`] ?? profile.dimensions;
            return sql`${sql(alias)}.embedding::halfvec(${dims}) <#> (${embeddingJson})::halfvec(${dims})`;
        };
        const _buildCtes = (input: { readonly embeddingJson?: string; readonly entityTypes: readonly string[]; readonly includeGlobal: boolean; readonly profile?: EmbeddingProfile; readonly scopeId: string | null; readonly term: string }) => {
            ...
            const hasEmbedding = input.embeddingJson !== undefined && input.profile !== undefined;
            ...
            const semanticCte = hasEmbedding
                ? sql`, semantic_candidates AS (
                    SELECT embeddings.entity_type, embeddings.entity_id, -(${_distance('embeddings', input.profile, input.embeddingJson)}) AS semantic_score
                    ...
                    WHERE ${where} AND embeddings.provider = ${input.profile?.provider} AND embeddings.model = ${input.profile?.model} AND embeddings.dimensions = ${input.profile?.dimensions} AND embeddings.embedding_hash = documents.document_hash
                    ORDER BY ${_distance('embeddings', input.profile, input.embeddingJson)}, embeddings.entity_id DESC LIMIT ${_cfg.limits.candidate})`
                : sql``;
        ...
        const _searchInput = (options: { readonly entityTypes?: readonly string[] | undefined; readonly includeFacets?: boolean | undefined; readonly includeGlobal?: boolean | undefined; readonly includeSnippets?: boolean | undefined; readonly scopeId: string | null; readonly term: string }, limit: number, cursor: Option.Option<{ id: string; v: number }>, embedding: Option.Option<{ embeddingJson: string; profile: EmbeddingProfile }>) => ({
            ...
            ...Option.getOrUndefined(Option.map(embedding, (e) => ({ dimensions: e.profile.dimensions, embeddingJson: e.embeddingJson, model: e.profile.model, provider: e.profile.provider }))),
        });
        const executeSearch = SqlSchema.findAll({
            execute: (input) => {
                ...
                const { ctes, query } = _buildCtes({ entityTypes: input.entityTypes, includeGlobal: input.includeGlobal, scopeId: input.scopeId, term: input.term,
                    ...
                    ...Option.getOrUndefined(Option.map(Option.all({ dimensions: Option.fromNullable(input.dimensions), model: Option.fromNullable(input.model), provider: Option.fromNullable(input.provider) }), (profile) => ({ profile }))),
                });
            ...
            Request: S.Struct({
                cursorId: S.optional(_SearchEntityId), cursorRank: S.optional(S.Number), dimensions: S.optional(S.Int), embeddingJson: S.optional(S.String),
                entityTypes: S.Array(S.String), includeFacets: S.Boolean, includeGlobal: S.Boolean, includeSnippets: S.Boolean,
                limit: S.Int.pipe(S.positive(), S.lessThanOrEqualTo(_cfg.limits.max)), model: S.optional(S.String), provider: S.optional(S.Literal('anthropic', 'gemini', 'openai')),
                scopeId: S.NullOr(_SearchScopeId), term: S.String.pipe(S.minLength(_cfg.limits.termMin), S.maxLength(_cfg.limits.termMax)),
            }).pipe(S.mutable),
            Result: S.Struct({
        ...
        const executeEmbeddingSources = SqlSchema.findAll({
            execute: (input) => {
                ...
                return sql`
                    SELECT documents.entity_type, documents.entity_id, documents.scope_id, documents.display_text, documents.content_text, documents.metadata, documents.document_hash, documents.normalized_text, documents.updated_at
                    FROM ${sql(_cfg.docsTable)} documents LEFT JOIN ${sql(_cfg.embeddingsTable)} embeddings
                     ...
                     AND embeddings.provider = ${input.provider} AND embeddings.model = ${input.model} AND embeddings.dimensions = ${input.dimensions}
                    WHERE ${where} AND (embeddings.entity_id IS NULL OR embeddings.embedding_hash IS DISTINCT FROM documents.document_hash)
                    ORDER BY documents.updated_at DESC LIMIT ${input.limit}`;
            },
            Request: S.Struct(_embedRequestFields),
        ...
        const executeUpsertEmbedding = SqlSchema.single({
            execute: (input) => sql`
                INSERT INTO ${sql(_cfg.embeddingsTable)} (entity_type, entity_id, provider, model, dimensions, embedding, embedding_hash)
                VALUES (${input.entityType}, ${input.entityId}, ${input.provider}, ${input.model}, ${input.dimensions}, (${input.embeddingJson})::halfvec, ${input.documentHash})
                ON CONFLICT (entity_type, entity_id, provider, model, dimensions) DO UPDATE
                SET embedding = EXCLUDED.embedding, embedding_hash = EXCLUDED.embedding_hash, updated_at = clock_timestamp()
                RETURNING entity_type, entity_id, provider, model, dimensions`,
            Request: S.Struct({ ..._embeddingProfileSchema.fields, documentHash: S.String, embeddingJson: S.String, entityId: _SearchEntityId, entityType: S.String }),
            Result:  S.Struct({ dimensions: S.Int, entityId: _SearchEntityId, entityType: S.String, model: S.String, provider: S.String }),
        });
        const executeStaleEmbeddings = SqlSchema.findAll({
            execute: (input) => {
                const where = _filters(input, 'documents');
                return sql`
                    SELECT embeddings.entity_type, embeddings.entity_id, embeddings.provider AS stale_provider, embeddings.model AS stale_model, embeddings.dimensions AS stale_dimensions
                    FROM ${sql(_cfg.embeddingsTable)} embeddings JOIN ${sql(_cfg.docsTable)} documents ON documents.entity_type = embeddings.entity_type AND documents.entity_id = embeddings.entity_id
                    WHERE ${where} AND NOT (embeddings.provider = ${input.provider} AND embeddings.model = ${input.model} AND embeddings.dimensions = ${input.dimensions})
                    ORDER BY embeddings.updated_at DESC LIMIT ${input.limit}`;
            },
            Request: S.Struct(_embedRequestFields),
            Result:  S.Struct({ entityId: _SearchEntityId, entityType: S.String, staleDimensions: S.Int, staleModel: S.String, staleProvider: S.String }),
        });
            ...
            upsertEmbedding: Effect.fn('SearchRepo.upsertEmbedding')((input: { readonly documentHash: string; readonly embedding: readonly number[]; readonly entityId: string; readonly entityType: string; readonly profile: EmbeddingProfile }) =>
                _embeddingPayload({ profile: input.profile, vector: input.embedding }).pipe(
                    Effect.andThen(({ embeddingJson, profile }) =>
                        executeUpsertEmbedding({ dimensions: profile.dimensions, documentHash: input.documentHash, embeddingJson, entityId: input.entityId, entityType: input.entityType, model: profile.model, provider: profile.provider }).pipe(_mapErr('upsertEmbedding'))))),
        };
    }),
}) {
    static readonly Test = (overrides: Partial<SearchRepo> = {}) => Layer.succeed(SearchRepo, {
        embeddingSources: () => Effect.succeed([]), refresh: () => Effect.void,
        ...
        upsertEmbedding: () => Effect.succeed({ dimensions: 1_536, entityId: '00000000-0000-0000-0000-000000000000', entityType: 'app' as const, model: 'text-embedding-3-small', provider: 'openai' }),
        ...overrides,
    } as SearchRepo);
}
