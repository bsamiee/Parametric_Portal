...
def _container_logs_for_name(pod: str, namespace: str, name: str) -> str:
    ...
    current = _kubectl(f"kubectl logs {pod} -n {namespace} -c {name} --tail=100")
    previous = _kubectl(f"kubectl logs {pod} -n {namespace} -c {name} --previous --tail=50 2>&1")
    previous_section = (
        "" if "previous terminated container" in previous.lower() else f"--- {name} (previous) ---\n{previous}"
    )
...
def status(pod: str, namespace: str, _container: str) -> str:
    ...
    phase = _kubectl(f"kubectl get pod {pod} -n {namespace} -o jsonpath='{{.status.phase}}'").strip()
    restart_count = _parse_restart_count(
        _kubectl(f"kubectl get pod {pod} -n {namespace} -o jsonpath='{{.status.containerStatuses[0].restartCount}}'")
    )
...
def events(pod: str, namespace: str, _container: str) -> str:
    ...
    return (
        _kubectl(
            f"kubectl get events -n {namespace} --field-selector involvedObject.name={pod} --sort-by='.lastTimestamp'"
        )
...
def logs(pod: str, namespace: str, _container: str) -> str:
    ...
    names = _kubectl(f"kubectl get pod {pod} -n {namespace} -o jsonpath='{{.spec.containers[*].name}}'").strip().split()
    return "\n".join(_container_logs_for_name(pod, namespace, name) for name in names)
...
def limits(pod: str, namespace: str, container: str) -> str:
    ...
    return _kubectl(
        f"kubectl get pod {pod} -n {namespace} -o jsonpath='{{.spec.containers[?(@.name==\"{container}\")].resources}}'"
    )
...
def node(pod: str, namespace: str, _container: str) -> str:
    ...
    node_name = _kubectl(f"kubectl get pod {pod} -n {namespace} -o jsonpath='{{.spec.nodeName}}'").strip()
    return (
        "Pod not scheduled" if not node_name else f"Node: {node_name}\n{_kubectl(f'kubectl top node {node_name} 2>&1')}"
    )
...
def env_vars(pod: str, namespace: str, container: str) -> str:
    ...
    return _kubectl(f"kubectl exec {pod} -n {namespace} -c {container} -- env 2>&1") or "[exec unavailable]"
