...
checks: dict[str, Check] = {
    ...
    "settings_caller_policy": lambda data: (
        lambda settings: [f"settings.callerPolicy invalid: {settings.get('callerPolicy')} (allowed: {DEFAULTS.caller_policy})"]
        if "callerPolicy" in settings and settings["callerPolicy"] not in DEFAULTS.caller_policy else []
    )(data.get("settings", {})),
    "settings_exec_order_ai": lambda data: (
        lambda has_ai, settings: [
            "AI workflow requires settings.executionOrder='v1'"
        ] if has_ai and settings.get("executionOrder") != "v1" else []
    )(
