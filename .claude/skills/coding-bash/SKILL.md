...
name: coding-bash
description: >-
  Generates and validates production-ready bash 5.2+/5.3 scripts with strict mode, immutable
  locals, dispatch tables, middleware composition, metadata-driven help, ERR traps, atomic I/O,
  and functional patterns. Use when creating, editing, reviewing, or validating .sh scripts,
  CLI tools, cron jobs, deployment automation, text processing workflows, or log analyzers.
  Also use when debugging ShellCheck SC codes or auditing shell best practices.
---
...
4. Read [text-processing-guide.md](./references/text-processing-guide.md) — Regex dialects, rg/awk/sd selection, pipeline patterns, performance.
5. Read [validation.md](./references/validation.md) — ShellCheck 0.11.0+ codes, directives, `.shellcheckrc`, SC-avoiding patterns, CI integration.
6. Structure script — Shebang + strict mode + shopt + readonly constants + ERR/EXIT traps + main.
7. Implement — Core functions, business logic, main entry point.
8. Validate — `bash -n script.sh`, ShellCheck 0.11.0+, re-validate until clean.
