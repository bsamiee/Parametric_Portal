...
from _checks import find_metrics
from _common import (
    COUNTER_SUFFIXES,
    GAUGE_PATTERNS,
    Finding,
    ...
    return [
        {
            ...
            'message': (
                f'predict_linear() with [{duration}{unit}] -- too few data points for reliable linear regression'
            ),
            'severity': 'warning',
            'recommendation': 'Use [10m]+ for sufficient data points; [1h]+ for production forecasts',
        }
        ...
        [
            {
                ...
                'message': 'Dimensions embedded in metric name -- reduces queryability and increases series count',
                'severity': 'info',
                'recommendation': 'Use labels: http_requests_total{method="GET", status="200"}',
            }
        ]
        ...
        [
            {
                ...
                'message': (
                    'histogram_quantile() on raw buckets -- rate() handles counter resets per-series before aggregation'
                ),
                'severity': 'warning',
                'recommendation': 'histogram_quantile(0.95, sum by (le) (rate(m_bucket[5m])))',
            }
        ]
        ...
        [
            {
                ...
                'message': (
                    'Classic histograms need "le" in by() -- without it, '
                    'bucket boundaries are lost producing incorrect percentiles'
                ),
                'severity': 'warning',
                'recommendation': 'sum by (job, le) (...)',
            }
        ]
        ...
        [
            {
                ...
                'message': (
                    'group_left/right without on()/ignoring() -- '
                    'Prometheus requires explicit label matching for many-to-one joins'
                ),
                'severity': 'error',
                'recommendation': 'Add on(label1, label2) before group_left/right',
            }
        ]
        if re.search(r'\b(group_left|group_right)\s*\(', query_lower)
        and 'on(' not in query_lower
        and 'ignoring(' not in query_lower
        else []
    )
    ...
    missing_rate = [
        {
            ...
            'message': (
                f'{func}() needs rate() input -- without it, you get cumulative counts instead of per-second rates'
            ),
            'severity': 'warning',
            'recommendation': f'{func}(rate(histogram_metric[5m]))',
        }
        ...
        [
            {
                ...
                'message': (
                    'Native histograms do not need "le" in aggregation -- '
                    '"le" is only for classic histograms with explicit bucket boundaries'
                ),
                'severity': 'info',
                'recommendation': 'Simplify: sum by (job) (rate(metric[5m]))',
            }
        ]
        ...
        [
            {
                ...
                'message': (
                    'histogram_count/sum typically need rate() -- '
                    'otherwise returns cumulative total, not per-second rate'
                ),
                'severity': 'info',
                'recommendation': 'histogram_count(rate(metric[5m]))',
            }
        ]
        ...
        [
            {
                ...
                'message': (
                    f'Mixed types ({", ".join(sorted(types))}) in arithmetic -- '
                    'different metric types have incompatible semantics'
                ),
                'severity': 'warning',
                'recommendation': 'Separate into distinct queries per metric type',
            }
        ]
